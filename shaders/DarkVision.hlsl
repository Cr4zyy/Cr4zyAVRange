...
float4 SFXDarkVisionPS(PS_INPUT input) : COLOR0
{
	...
	float meleeRange = max(range - depth, 0); // 1.8
	...
	//set up screen center colouring
		...
		((model * (edge + edge)) * (lerp(meleeRange * colourBite, inputPixel, clamp(biteAreaX + biteAreaY, 0, 1))) * (1 + 0.3 * pow(0.1 + sin(time * 5 + intensity * .2), 2)) );
	...
	//outlines for when av is off, edges only
		...
		((model * (edge + edge)) * (lerp(meleeRange * disabledBite, inputPixel, clamp(biteAreaX + biteAreaY, 0, 1))) * (1 + 0.3 * pow(0.1 + sin(time * 5 + intensity * .2), 2)) ));
